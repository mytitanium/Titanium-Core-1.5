...
{
    const char* pszTimestamp = "DASH + RVN = TTM. Titanium is a decentralized public blockchain providing fast, cheap and secure transactions";
...
public:
    CMainParams() {
        ...
        consensus.nRuleChangeActivationThreshold = 1499; // 95% of 2016
        ...
        consensus.defaultAssumeValid = uint256S("0x0000028044198672d9926e9b0c49aafca41adb06278f48b000e2b369817a781c");
		...
		vSeeds.emplace_back("207.180.196.250");
		vSeeds.emplace_back("207.180.202.230");
		vSeeds.emplace_back("207.180.200.79");
		vSeeds.emplace_back("164.68.114.194");
		vSeeds.emplace_back("144.91.109.177");
		vSeeds.emplace_back("95.111.244.153");
		vSeeds.emplace_back("144.91.99.45");
		vSeeds.emplace_back("178.238.228.116");
...
public:
    CTestNetParams() {
        ...
        vSeeds.emplace_back("testnet-seed.ttmdot.io"); // Just a static list of stable node(s), only supports x9

        // Testnet Ttm addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Testnet Ttm script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Testnet private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Testnet Ttm BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet Ttm BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet Ttm BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
...
public:
    CDevNetParams() {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("ttmevo.org",  "devnet-seed.ttmevo.org"));
...
public:
    CRegTestParams() {
        ...
        // Regtest Ttm addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest Ttm script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Regtest Ttm BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Ttm BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Ttm BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
